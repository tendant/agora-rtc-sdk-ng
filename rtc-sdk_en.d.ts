...
export declare interface ClientConfig {
     ...
     *
     * If [mode]{@link ClientConfig.mode} is set as `live`, use this property to set the user role. The default role is `audience`.
     *
 ...
 *
 * - You can get the audio sampling device information through [AgoraRTCClient.on("microphone-changed")]{@link IAgoraRTCClient.event_microphone_changed}.
 * - You can get the video capture device information through [AgoraRTCClient.on("camera-changed")]{@link IAgoraRTCClient.event_camera_changed}.
 */
 ...
 *
 * In most cases, you only need to listen for [user-published]{@link IAgoraRTCClient.event_user_published} and [user-unpublished]{@link IAgoraRTCClient.event_user_unpublished} events for operations including subscribeing, unsubscribing, and displaying whether the remote user turns on the camera and mircophone. You do not need to pay special attention to user states since the SDK automatically handles user states.
 *
...
/**
 * The entry point of the Agora Web SDK NG.
 */
export declare interface IAgoraRTC {
    /**
     * The version of the Agora Web SDK NG.
     */
    VERSION: string;
    /**
     * Gets the codecs that the browser supports.
     *
     * This method gets a list of the codecs supported by the SDK and the web browser. The Agora Web SDK NG supports video codecs VP8 and H.264, and audio codec OPUS.
     *
     * > Note:
     ...
     *
     * This is usually the first step of using the Agora Web SDK NG.
     ...
     *
     * In the Agora Web SDK NG, once the user has interacted with the webpage, the autoplay blocking is removed. You can deal with the issue in either of the following ways:
...
export declare interface IAgoraRTCClient extends EventEmitter {
     ...
     * > - An `AgoraRTCClient` object can publish multiple audio tracks but only one video track. The SDK automatically mixes the audio tracks into one audio track.
     ...
     * > - In the communication profile ([mode]{@link ClientConfig.mode} is set as `rtc`), this method does not take effect. All users are `host` by default.
     ...
     *
     * To use the cloud proxy service, extra settings are needed. See [Use Cloud Proxy](https://docs.agora.io/en/Interactive%20Broadcast/cloud_proxy_web?platform=Web) for details.
     *
     * @param use443OnlyMode Set this parameter as `true` to transmit data only through the TCP 443 ports in scenarios when the firewall only opens TCP 443 and 80 ports. The default vaule is `false`.
     * @category Proxy
     */
    startProxyServer(use443OnlyMode?: boolean): void;
    /**
     ...
     * @param secret The encryption secret. ASCII characters only.
     */
     ...
     *
     * > Ensure that you [enable the RTMP Converter service](/docs/en/cdn_streaming/#prerequisites) before using this function.
     ...
     *
     * See [Push Streams to the CDN](/docs/en/cdn_streaming) for details.
     *
     * > Note:
     ...
     *
     * After you call this method, the server pulls the online stream and injects it into a live-broadcast channel. This is applicable to scenarios where all audience members in the channel can watch a live show and interact with each other. See [Inject Online Media Stream](/docs/en/inject_stream) for details.
     *
...
export declare interface ICameraVideoTrack extends ILocalVideoTrack {
     ...
     *
     * > Disabling a track does not trigger the [LocalTrack.on("track-ended")]{@link LocalTrack.event_track_ended} event.
...
export declare interface ILocalAudioTrack extends ILocalTrack {
     ...
     * Gets the statistics of a local audio track.
     *
     * @return An [[LocalAudioTrackStats]] object.
     */
...
export declare interface ILocalTrack extends ITrack {
     ...
     * @return A [[LocalVideoTrackStats]] or [[LocalAudioTrackStats]] object.
     */
...
export declare interface ILocalVideoTrack extends ILocalTrack {
     ...
     * Gets the statistics of a local video track.
     *
     * @return A [[LocalVideoTrackStats]] object.
     */
...
/**
 * Configurations for the watermark images to put on top of the video in [LiveStreamingTranscodingConfig]{@link LiveStreamingTranscodingConfig}.
 */
export declare interface LiveStreamingTranscodingImage {
    /**
     * The HTTP/HTTPS URL address of the watermark image on the broadcasting video.
     *
    ...
    height?: number;
    /**
     * The transparency level of the watermark image.
     *
...
/**
 * Information of the local audio track, which can be retrieved by calling [LocalAudioTrack.getStats]{@link ILocalAudioTrack.getStats}.
 */
...
/**
 * Information of the local video track, which can be retrieved by calling [LocalVideoTrack.getStats]{@link ILocalVideoTrack.getStats}.
 */
 ...
 *
 * After the user joins the channel, the SDK triggers the [AgoraRTCClient.on("network-quality")]{@link IAgoraRTCClient.event_network_quality} callback once every two seconds and provides the uplink and downlink last-mile network conditions of the user through this interface.
 */
...
/**
 * Statistics of the remote audio track, such as connection and transmission statistics, which can be retrieved by calling [RemoteAudioTrack.getStats]{@link IRemoteAudioTrack.getStats}.
 */
...
/**
 * Statistics of the remote video track, such as connection and transmission statistics, which can be retrieved by calling [RemoteVideoTrack.getStats]{@link IRemoteVideoTrack.getStats}.
 */
...
export declare interface ScreenVideoTrackInitConfig {
     ...
     * - Pass your customized audio encoder configurations by using [[VideoEncoderConfiguration]].
     */
